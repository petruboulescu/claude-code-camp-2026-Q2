...
class LoaderTest(unittest.TestCase):
    ...
    def test_repl_selection_forwards_tui_flag(self):
        package = unittest.mock.Mock()
        package.repl = unittest.mock.Mock()
        with (
            patch("boukensha_loader.resolve", return_value=(None, self.home, {})),
            patch("boukensha_loader.load_implementation", return_value=package),
        ):
            boukensha_loader.load_and_start_repl(argv=["--no-tui"], environ={})
            package.repl.assert_called_once_with(tui=False)
            package.repl.reset_mock()
            boukensha_loader.load_and_start_repl(argv=[], environ={})
            package.repl.assert_called_once_with(tui=True)
