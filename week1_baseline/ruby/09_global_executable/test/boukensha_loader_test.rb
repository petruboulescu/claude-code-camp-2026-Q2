...
class BoukenshaLoaderTest < Minitest::Test
  ...
  def test_version_prints_the_installed_version
    ...
    BoukenshaLoader.version(io: output)

    assert_equal "boukensha 0.9.0\n", output.string
  end

  def test_doctor_reports_paths_and_configuration_without_secret
    write_rc("boukensha_dir: config\n")
    config = File.join(@home, "config")
    FileUtils.mkdir_p(config)
    File.write(
      File.join(config, "settings.yaml"),
      "tasks:\n  player:\n    provider: openai\n    model: gpt-test\n"
    )
    File.write(File.join(config, ".env"), "OPENAI_API_KEY=super-secret\n")
    output = StringIO.new

    BoukenshaLoader.doctor(io: output)

    assert_includes output.string, "provider:          openai"
    assert_includes output.string, "model:             gpt-test"
    assert_includes output.string, "credential ready:  yes"
