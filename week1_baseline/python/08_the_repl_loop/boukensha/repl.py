  ...
  /help    show this message"""
    ...
    def _banner(self):
        if self.provider == "ollama":
            key_status = "API key not required"
        elif self.api_key is None or not str(self.api_key).strip():
        ...
        if config_dir is not None and config_dir.is_dir():
            config_line = str(config_dir)
        else:
            path = str(config_dir) if config_dir is not None else "(default)"
            config_line = f"{path}  ✗ directory not found"
