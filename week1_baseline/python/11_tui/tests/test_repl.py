...
from boukensha.repl import CommandResult, Repl
...
class ReplTest(unittest.TestCase):
    ...
    def test_output_callback_and_explicit_command_results(self):
        repl, _context, _logger, output = self.make_repl("")
        routed = []
        repl.on_output(routed.append)
        self.assertIs(CommandResult.NOT_COMMAND, repl.handle_command("hello"))
        self.assertIs(CommandResult.COMMAND, repl.handle_command("/help"))
        self.assertIn("Commands:", routed[-1])
