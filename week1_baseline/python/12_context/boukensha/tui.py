...
def context_severity(percent: int) -> str:
    if percent >= 85:
        return "alert"
    if percent >= 70:
        return "warning"
...
class Tui(App):
    ...
    def on_tui_log_event(self, message: LogEvent) -> None:
        if message.turn_id == self._turn_id:
            ...
            if message.event.get("phase") == "compaction":
                dropped = message.event.get("dropped", 0)
    ...
    def _refresh_lines(self) -> None:
        ...
        if self.progress.active:
            ...
            line = (f"  [ready]   ctx {format_tokens(context.current_tokens)} / "
                    f"{format_tokens(context.context_window)} ({context.usage_pct}%)   "
                    f"{self.progress.turns} turns")
        progress_widget = self.query_one("#progress", Static)
        if self.progress.active:
            progress_widget.styles.color = "cyan"
        else:
            progress_widget.styles.color = {
                "normal": "#808080", "warning": "#ffcc00", "alert": "#ff5555"
            }[context_severity(self.repl.context.usage_pct)]
        progress_widget.update(line)
        model = self.repl.model or "(model)"
        context = self.repl.context
        warning = " ⚠" if context_severity(context.usage_pct) == "alert" else ""
        status = (f" boukensha v{self.repl.version or '?.?.?'} · {model} · "
                  f"ctx {format_tokens(context.current_tokens)}/{format_tokens(context.context_window)} "
                  f"({context.usage_pct}%){warning} · "
